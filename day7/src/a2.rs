...
pub fn compare_set_and_high(a: &Hand, b: &Hand) -> std::cmp::Ordering {
    ...
    if a_score.len() > 1 {
        if let Some(j_value) = a_score.remove(&Card::Jack) {
            let card_with_max_value = a_score.iter().max_by_key(|(_, &v)| v).map(|(&k, _)| k);
            if let Some(card) = card_with_max_value {
                *a_score.entry(card).or_insert(0) += j_value;
            }
        }
    }

    if b_score.len() > 1 {
        if let Some(j_value) = b_score.remove(&Card::Jack) {
            let card_with_max_value = b_score.iter().max_by_key(|(_, &v)| v).map(|(&k, _)| k);
            if let Some(card) = card_with_max_value {
                *b_score.entry(card).or_insert(0) += j_value;
            }
        }
    }
